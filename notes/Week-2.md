...
Notice we called the `PrintName` twice. The first one is the **prototype**, to tell `{c}lang` that the function exists somewhere.
...
## Typecasting
**Typecasting** is converting one data type to another. For example, the `{c}har` A is equivalent to the `int` 65, its ASCII representation.
